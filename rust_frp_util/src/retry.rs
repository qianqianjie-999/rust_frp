//! 重试机制模块
//!
//! 该模块实现了 FRP 的重试功能，用于处理临时性网络故障和连接问题。
//!
//! ## 重试策略
//!
//! ```text
...
//! attempt: 0    1      2        3          4
//!          │    │      │        │          │
//! delay:   0   100ms  200ms    400ms      800ms
//!          │    │      │        │          │
//!          └────┴──────┴────────┴──────────┴──> 时间
//!
//! 最大延迟限制：30秒
...
//!
//! 1. **指数退避**
//!    - 每次重试的延迟时间乘以 backoff_multiplier
//!    - 避免频繁重试造成服务器压力
//!
//! 2. **最大延迟限制**
//!    - 防止延迟无限增长
...
//!
//! 3. **可重试错误判断**
//!    - ConnectionRefused: 服务器未启动，可重试
//!    - ConnectionReset: 连接被重置，可重试
//!    - Timeout: 连接超时，可重试
//!    - NotConnected: 未连接，可重试
//!
//! 4. **统计信息**
//!    - 返回尝试次数
//!    - 返回总延迟时间
